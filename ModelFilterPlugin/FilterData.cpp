...
GroupData FilterData::parseGroup(QXmlStreamReader& reader)
{
  ...
  while (!(reader.tokenType() == QXmlStreamReader::EndElement && reader.name() == "group"))
  {
    if (reader.tokenType() == QXmlStreamReader::StartElement)
    {
      ...
      {
        SearchÑriteriaData propertyData = parseProperty(reader, groupData.m_groupType);
...
SearchÑriteriaData FilterData::parseProperty(QXmlStreamReader& reader, rengaapi::ObjectType type)
{
  // parse property 
  reader.readNext();
  ...
  if (ok && propertyName.length() > 0)
    return SearchÑriteriaData(Property(propertyType, valueType, propertyName), operatorType, value);
  else
    // property not valid
    return SearchÑriteriaData(Property(PropertyType::Invalid, valueType, propertyName), operatorType, value);
}
